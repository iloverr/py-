# 完善功能：1.撞墙死亡 2.撞自己死亡 3.按R键重新开始 4.游戏结束画面提示 5.优化游戏视图，添加了新的背景图片、食物图片、小猪头图片、猪屁股图片 6.添加了"王泽中"文字显示 7.优化了蛇身绘制方式，使用圆形代替矩形 8.优化了蛇头和蛇尾的贴图旋转，使其朝向与移动方向一致 9.添加分数显示和最高分记录 10.分数达到30分后显示成功画面
...
success_img = pygame.image.load(os.path.join(PIC_DIR, "成功.png"))  # 加载游戏成功时显示的图片
success_img = pygame.transform.scale(success_img, (WIDTH, HEIGHT))  # 将成功图片缩放到窗口大小
...
score_font = pygame.font.SysFont("SimHei", 28, bold=True)  # 创建小号字体，用于显示分数和最高分
...
def reset_game():  # 定义重置游戏函数，用于游戏结束后按R键重新开始
    global snake, direction, food, game_over, move_timer, score  # 声明要修改的全局变量
...
while running:  # 游戏主循环，游戏的核心运行逻辑都在这个循环内
    ...
    if move_timer >= MOVE_INTERVAL and not game_over:  # 如果计时器达到1秒间隔且游戏未结束
        ...
        if new_head[0] < 0 or new_head[0] >= COLS or new_head[1] < 0 or new_head[1] >= ROWS:  # 判断新蛇头是否超出地图边界（0~11）
            ...
            if new_head == food:  # 判断新蛇头是否吃到了食物
                ...
                score += 1  # 得分加1
                if score > high_score:  # 如果当前分数超过最高分
    ...
    score_text = score_font.render(f"分数: {score}", True, (255, 255, 0))  # 渲染当前分数文字，黄色
    score_rect = score_text.get_rect(center=(WIDTH // 2, 30))  # 将分数文字居中显示在屏幕上方
    screen.blit(score_text, score_rect)  # 绘制分数文字

    high_text = score_font.render(f"最高分: {high_score}", True, (255, 200, 100))  # 渲染最高分文字，橙色
    high_rect = high_text.get_rect(center=(WIDTH // 2, 65))  # 将最高分文字居中显示在分数下方
    screen.blit(high_text, high_rect)  # 绘制最高分文字

    target_text = score_font.render("目标30分", True, (200, 200, 200))  # 渲染目标文字，浅灰色
    target_rect = target_text.get_rect(center=(WIDTH // 2, 100))  # 将目标文字居中显示在最高分下方
    screen.blit(target_text, target_rect)  # 绘制目标文字

    if game_over:  # 如果游戏结束标志为True
        if score >= 30:  # 如果分数达到30分及以上，显示成功画面
            screen.blit(success_img, (0, 0))  # 绘制成功全屏图片
            go_text = font.render("游戏成功，小猪吃的好开心", True, (0, 100, 255))  # 渲染成功文字，蓝色
            go_text2 = font.render("继续挑战最高分吧", True, (0, 100, 255))  # 渲染第二行成功文字，蓝色
            screen.blit(go_text, (WIDTH // 2 - go_text.get_width() // 2, HEIGHT // 2 - 40))  # 成功文字居中显示
            screen.blit(go_text2, (WIDTH // 2 - go_text2.get_width() // 2, HEIGHT // 2 + 10))  # 第二行文字在下方
            restart_text = font.render("按R键重新开始", True, (255, 255, 0))  # 渲染重新开始提示，黄色
            screen.blit(restart_text, (WIDTH // 2 - restart_text.get_width() // 2, HEIGHT // 2 + 60))  # 重新开始提示
        else:  # 分数不足30分，显示失败画面
            screen.blit(fail_img, (0, 0))  # 在窗口(0,0)位置绘制失败图片，覆盖整个窗口作为失败背景
            go_text = font.render("猪猪失败了", True, (173, 216, 230))  # 渲染"猪猪失败了"文字，使用浅蓝色
            restart_text = font.render("按R键重新开始", True, (255, 255, 0))  # 渲染"按R键重新开始"文字，使用黄色
            screen.blit(go_text, (WIDTH // 2 - go_text.get_width() // 2, HEIGHT // 2 - 40))  # 将失败文字水平居中显示
            screen.blit(restart_text, (WIDTH // 2 - restart_text.get_width() // 2, HEIGHT // 2 + 10))  # 将重新开始提示水平居中
